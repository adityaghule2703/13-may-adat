...
const Dashboard = () => {
  ...
  // Separate state for buyers summary
  const [buyersSummary, setBuyersSummary] = useState({
    totalBuyers: 0,
    activeBuyers: 0,
    totalPurchaseValue: 0,
  ...
  // Sample data for graphs (fallback)
  const weeklyData = {
    labels: [t('dashboard.days.mon'), t('dashboard.days.tue'), t('dashboard.days.wed'), t('dashboard.days.thu'), t('dashboard.days.fri'), t('dashboard.days.sat'), t('dashboard.days.sun')],
    purchase: [85000, 92000, 78000, 105000, 125000, 145000, 135000],
    sale: [110000, 125000, 98000, 142000, 185000, 210000, 195000]
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      if (data.success && data.data) {
        setDashboardData({
          summary: {
            totalFarmers: data.data.summary?.totalFarmers || 0,
            totalPayments: data.data.summary?.totalPayments || 0,
            totalWarehouses: data.data.summary?.totalWarehouses || 0,
...
const fetchBuyersSummary = async () => {
  ...
  try {
    ...
    console.log('Buyers Summary Response:', data); // Debug log
    
    if (data.success) {
      const topBuyersList = data.data?.topBuyers || [];
      const firstTopBuyer = topBuyersList.length > 0 ? topBuyersList[0] : null;
      
      setBuyersSummary({
        totalBuyers: data.data?.totalBuyers || 0,
        activeBuyers: data.data?.activeBuyers || 0,
        totalPurchaseValue: data.data?.totalPurchaseValue || 0,
        topBuyerOfMonth: firstTopBuyer ? {
          exists: true,
          buyerId: firstTopBuyer._id || firstTopBuyer.id,
          name: firstTopBuyer.displayName || firstTopBuyer.businessName || firstTopBuyer.name,
          displayName: firstTopBuyer.displayName,
          businessName: firstTopBuyer.businessName,
          totalPurchases: firstTopBuyer.totalPurchases || 0,
          totalValue: firstTopBuyer.totalPurchaseValue || 0
        } : {
          exists: false,
          buyerId: null,
          name: "Unknown",
  ...
  const getTransactionIcon = (type) => {
    switch(type?.toLowerCase()) {
      case 'purchase':
        return <ShoppingCart className="w-4 h-4" style={{ color: '#2E7D32' }} />;
      case 'sale':
        return <TrendingUp className="w-4 h-4" style={{ color: '#FF6F00' }} />;
      default:
        return <Activity className="w-4 h-4" style={{ color: '#8D6E63' }} />;
    }
  };

  // Transform graphData for chart display
  const getChartData = () => {
    if (dashboardData.graphData && dashboardData.graphData.length > 0) {
      const labels = dashboardData.graphData.map(item => item.day || item.date?.split('-')[2]);
      const purchaseData = dashboardData.graphData.map(item => item.purchases?.total || 0);
      const saleData = dashboardData.graphData.map(item => item.sales?.total || 0);
      
      return { labels, purchase: purchaseData, sale: saleData };
    }
  ...
  const stats = [
    { 
      title: t('dashboard.stats.totalFarmers'), 
      value: formatNumber(dashboardData.summary.totalFarmers), 
      change: "+12", 
      changeType: "increase",
      icon: Leaf, 
      color: "#2E7D32",
    ...
    { 
      title: t('dashboard.stats.totalWarehouses'), 
      value: formatNumber(dashboardData.summary.totalWarehouses), 
      change: "+8%", 
      changeType: "increase",
      icon: ShoppingCart, 
      color: "#FF6F00",
  ...
  if (loading && dashboardData.summary.totalFarmers === 0 && dashboardData.summary.totalRevenue === 0) {
    return (
      <div className="flex items-center justify-center h-96">
        <div className="text-center">
          <RefreshCw className="w-10 h-10 animate-spin mx-auto mb-4" style={{ color: '#2E7D32' }} />
  ...
  const topBuyer = buyersSummary.topBuyerOfMonth;
  const chartData = getChartData();
    ...
    <div className="space-y-6">
      ...
      {error && (
        <div className="bg-red-50 border border-red-200 rounded-xl p-4 flex items-center gap-3">
          <AlertCircle className="w-5 h-5 text-red-500" />
          <span className="text-sm text-red-600">{error}</span>
          <button onClick={fetchAllData} className="ml-auto text-sm text-red-600 hover:underline">
            {t('common.retry')}
          </button>
        </div>
      )}
                  ...
                  return (
                    <div key={idx}>
                      <div className="flex justify-between text-xs mb-1">
                        <span style={{ color: '#8D6E63' }}>{label}</span>
                        <div className="flex gap-3">
                          <span style={{ color: '#2E7D32' }}>{t('dashboard.chart.purchase')}: {formatCurrency(purchaseValue)}</span>
                          <span style={{ color: '#FF6F00' }}>{t('dashboard.chart.sale')}: {formatCurrency(saleValue)}</span>
                        </div>
                      </div>
                      <div className="flex gap-1 h-8">
                        <div 
        ...
        <div className="space-y-6">
          <div className="bg-gradient-to-br from-green-600 to-green-700 rounded-2xl p-5 text-white shadow-sm">
            <div className="flex justify-between items-start">
              <div>
                <p className="text-xs opacity-90">{t('dashboard.stats.totalRevenue')}</p>
                <p className="text-2xl font-bold mt-1">{formatCurrency(dashboardData.summary.totalRevenue)}</p>
                <p className="text-xs mt-2 opacity-80">{t('dashboard.fromLastMonth')}</p>
              </div>
              <Wallet className="w-8 h-8 opacity-90" />
            </div>
          ...
          <div className="bg-white rounded-2xl p-5 shadow-sm">
            ...
            <div className="space-y-3">
              {dashboardData.topProducts.slice(0, 3).map((product, i) => (
                <div key={product.id || i} className="flex justify-between items-center">
                  <div>
                    <p className="text-sm font-medium" style={{ color: '#2E7D32' }}>{product.name}</p>
                    <p className="text-xs" style={{ color: '#8D6E63' }}>
                      {product.currentStock} {product.unit} • {product.warehouse}
                    </p>
                  </div>
              ...
              {dashboardData.topProducts.length === 0 && (
                <p className="text-sm text-center" style={{ color: '#8D6E63' }}>{t('dashboard.noProductsFound')}</p>
              )}
            </div>
          </div>
        </div>
      ...
      {/* Recent Transactions & Top Buyer Section */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {/* Recent Transactions */}
        <div className="bg-white rounded-2xl shadow-sm overflow-hidden">
          <div className="px-6 py-4 border-b flex justify-between items-center" style={{ borderColor: '#E8F5E9' }}>
            <div>
              <h3 className="font-semibold text-lg" style={{ color: '#1B5E20' }}>{t('dashboard.recentTransactions')}</h3>
              <p className="text-xs mt-1" style={{ color: '#8D6E63' }}>{t('dashboard.recentTransactionsSubtitle')}</p>
            </div>
          </div>
          <div className="divide-y" style={{ borderColor: '#E8F5E9' }}>
            {dashboardData.recentTransactions.length === 0 ? (
              <div className="px-6 py-8 text-center">
                ...
                <p className="text-sm" style={{ color: '#8D6E63' }}>{t('dashboard.noTransactionsFound')}</p>
              </div>
            ) : (
                ...
                <div key={transaction.id} className="px-6 py-3 hover:bg-green-50 transition-all">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="text-sm font-medium" style={{ color: '#2E7D32' }}>
                          {transaction.partyName || t('dashboard.unknown')}
                        </p>
                        <p className="text-xs" style={{ color: '#8D6E63' }}>
                          {transaction.formattedDate || formatDate(transaction.date)} • {transaction.typeLabel || transaction.type}
                          {transaction.referenceNumber && ` • ${transaction.referenceNumber}`}
                        </p>
                      </div>
                    </div>
                    <div className="text-right">
                      <p className="text-sm font-bold" style={{ color: transaction.type === 'purchase' ? '#2E7D32' : '#FF6F00' }}>
                      ...
                      <span className={`text-xs px-2 py-0.5 rounded-full ${getStatusColor(transaction.status)}`}>
                        {transaction.status?.toUpperCase() || t('dashboard.completed')}
                      </span>
                    </div>
                  </div>
                </div>
        ...
        {/* Top Buyer Section - Using Buyers Summary API */}
        <div className="bg-gradient-to-br from-orange-500 to-orange-600 rounded-2xl p-6 text-white shadow-sm">
          <div className="flex justify-between items-start mb-4">
            <div>
              <div className="flex items-center gap-2 mb-2">
              ...
              {topBuyer && topBuyer.exists && topBuyer.name !== "Unknown" ? (
                <>
                  <p className="text-2xl font-bold mt-1">{topBuyer.name}</p>
                  <p className="text-sm mt-1 opacity-90">
                    {topBuyer.totalPurchases} {t('dashboard.purchases')}
                  </p>
                </>
              ) : (
                <p className="text-xl font-bold mt-1">{t('dashboard.noBuyersYet')}</p>
          ...
          {topBuyer && topBuyer.exists && topBuyer.name !== "Unknown" && (
            <div className="grid grid-cols-2 gap-4 mt-4">
              ...
              <div className="bg-white/20 backdrop-blur rounded-xl p-3">
                <p className="text-xs opacity-80">{t('dashboard.purchaseValue')}</p>
                <p className="text-xl font-semibold mt-1">{formatCurrency(topBuyer.totalValue || 0)}</p>
              </div>
            </div>
          )}
