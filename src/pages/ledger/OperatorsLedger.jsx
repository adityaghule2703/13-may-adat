// src/pages/ledger/OperatorsLedger.jsx
import React, { useState, useEffect, useCallback } from 'react';
import { useNavigate } from 'react-router-dom';
import { useTranslation } from 'react-i18next';
import {
  BookOpen, Search, Filter, Loader, AlertCircle,
  User, Mail, Phone, X, Eye, Printer, TrendingUp, TrendingDown, DollarSign,
  Users, MoreVertical
} from 'lucide-react';
import BASE_URL from '../../config/Config';

const OperatorsLedger = ({ searchTerm, debouncedSearchTerm, refreshTrigger }) => {
  ...
  }, [fetchOperatorsLedger, refreshTrigger]);
  ...
  const totalPages = Math.ceil(filteredOperators.length / pagination.limit);
    ...
    <>
        ...
        <div className="flex flex-wrap gap-4 items-center justify-between">
          <div className="w-80">
            <div className="relative">
              ...
              <input
                type="text"
                placeholder={t('ledger.search.operatorsPlaceholder')}
                value={searchTerm}
                onChange={(e) => {}}
      ...
      <div className="bg-white rounded-xl shadow-sm overflow-hidden">
        {loading ? (
          ...
          <div className="text-center py-12"><BookOpen className="w-12 h-12 mx-auto mb-3" style={{ color: '#C8E6C9' }} /><p className="text-sm" style={{ color: '#8D6E63' }}>{t('ledger.noOperatorsFound')}</p></div>
        ) : (
          <>
              ...
              <table className="w-full">
                ...
                <tbody>
                  {paginatedOperators.map((operatorData, idx) => {
                    ...
                    const netProfit = financialSummary.netProfit || 0;
                    const profitColor = netProfit >= 0 ? '#2E7D32' : '#D32F2F';
                    const isItemActionMenuOpen = isActionMenuOpen && selectedItem?.operator?.id === operator.id;
                    return (
                      <tr key={operator.id} className="hover:bg-green-50 transition-colors" style={{ borderBottom: idx !== paginatedOperators.length - 1 ? '1px solid #E8F5E9' : 'none' }}>
                        ...
                        <td className="px-6 py-4 text-center">
                          <button onClick={(e) => handleActionMenuOpen(e, operatorData)} className="p-2 rounded-lg hover:bg-gray-100 transition-all flex items-center gap-1 mx-auto" style={{ color: '#2E7D32' }}><MoreVertical className="w-4 h-4" /><span className="text-xs font-medium">{t('common.actions')}</span></button>
                          {isItemActionMenuOpen && anchorRect && (
                            <div className="fixed bg-white rounded-lg shadow-xl border overflow-hidden z-50" style={{ borderColor: '#E8F5E9', width: '180px', position: 'fixed', top: openUpward ? anchorRect.top - MENU_HEIGHT - 4 : anchorRect.bottom + 4, left: anchorRect.left - 100 }}>
                              <button onClick={() => handleViewDetails(operatorData)} className="w-full px-4 py-2.5 text-left text-sm hover:bg-green-50 flex items-center gap-2 transition-colors" style={{ color: '#2E7D32' }}><Eye className="w-4 h-4" />{t('common.viewDetails')}</button>
                              <button onClick={() => handlePrintSummary(operatorData)} className="w-full px-4 py-2.5 text-left text-sm hover:bg-gray-50 flex items-center gap-2 transition-colors border-t" style={{ color: '#1565C0', borderColor: '#E8F5E9' }}><Printer className="w-4 h-4" />{t('ledger.buttons.printSummary')}</button>
                            </div>
                          )}
                        </td>
                      </tr>
...
export default OperatorsLedger;