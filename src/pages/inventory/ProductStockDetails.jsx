...
//         <button onClick={() => navigate('/inventory')} className="mt-4 px-4 py-2 rounded-lg text-white text-sm transition-all hover:scale-105" style={{ background: '#2E7D32' }}>
//           Back to Inventory
//         </button>
//       </div>
//     );
...
//           <button onClick={() => navigate('/inventory')} className="p-2 rounded-lg hover:bg-gray-100 transition-colors">
...
//           <h2 className="text-sm font-semibold uppercase tracking-wider" style={{ color: '#FFFFFF' }}>Stock by Warehouse</h2>
//         </div>
...
//                 <th className="px-6 py-3 text-left text-xs font-semibold uppercase tracking-wider" style={{ color: '#5D4037' }}>Status</th>
//                 <th className="px-6 py-3 text-left text-xs font-semibold uppercase tracking-wider" style={{ color: '#5D4037' }}>Last Updated</th>
//                 <th className="px-6 py-3 text-left text-xs font-semibold uppercase tracking-wider" style={{ color: '#5D4037' }}>Actions</th>
//               </tr>
//             </thead>
//             <tbody>
...
//                         <span className="font-medium text-sm" style={{ color: '#2E7D32' }}>{item.warehouse}</span>
//                       </div>
//                     </td>
//                     <td className="px-6 py-4 whitespace-nowrap">
...
const ProductStockDetails = () => {
  ...
  const getStockStatus = (stock) => {
    if (stock <= 0) return { color: '#D32F2F', bg: '#FFEBEE', label: t('inventory.status.outOfStock'), icon: XCircle };
    if (stock <= 10) return { color: '#FF6F00', bg: '#FFF3E0', label: t('inventory.status.lowStock'), icon: AlertTriangle };
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center h-96">
        <Loader className="w-8 h-8 animate-spin" style={{ color: '#2E7D32' }} />
        <span className="ml-2" style={{ color: '#2E7D32' }}>{t('inventory.loadingStockDetails')}</span>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center flex-wrap gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold" style={{ color: '#1B5E20' }}>{productName}</h1>
            <p className="text-sm mt-1" style={{ color: '#8D6E63' }}>{t('inventory.stockDetailsSubtitle')}</p>
          </div>
        </div>
        <div className="flex gap-2">
          <button 
      ...
      <div className="bg-white rounded-xl shadow-sm overflow-hidden">
        <div className="px-6 py-3 border-b" style={{ background: '#1B3A1F', borderColor: '#2E5A32' }}>
          <h2 className="text-sm font-semibold uppercase tracking-wider" style={{ color: '#FFFFFF' }}>{t('inventory.stockByWarehouse')}</h2>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead>
              <tr style={{ background: '#FAFAFA', borderBottom: '1px solid #E8F5E9' }}>
                <th className="px-6 py-3 text-left text-xs font-semibold uppercase tracking-wider" style={{ color: '#5D4037' }}>{t('inventory.table.warehouse')}</th>
                ...
                <th className="px-6 py-3 text-left text-xs font-semibold uppercase tracking-wider" style={{ color: '#5D4037' }}>{t('inventory.table.status')}</th>
                ...
                <th className="px-6 py-3 text-left text-xs font-semibold uppercase tracking-wider" style={{ color: '#5D4037' }}>{t('inventory.table.actions')}</th>
              </tr>
            </thead>
            <tbody>
              {stockData.map((item, index) => {
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex gap-2">
                        <button 
                          onClick={() => navigate(`/inventory/adjust?product=${item.productName}&warehouse=${item.warehouse}`)}
                          className="p-1 rounded hover:bg-gray-100 transition-colors" 
                          title={t('inventory.buttons.adjustStock')}
                        >
                          <TrendingUp className="w-4 h-4" style={{ color: '#FF6F00' }} />
                        </button>
                        <button 
                          onClick={() => navigate(`/inventory/transfer?product=${item.productName}&from=${item.warehouse}`)}
                          className="p-1 rounded hover:bg-gray-100 transition-colors" 
                          title={t('inventory.buttons.transferStock')}
                        >
                          <ArrowLeftRight className="w-4 h-4" style={{ color: '#1976D2' }} />
                        </button>
                      </div>
