// src/pages/buyers/EditBuyer.jsx
...
// Color constants matching AddBuyer
const COLORS = {
  primary: '#1B3A1F',
  primaryLight: '#E8F5E9',
  primaryDark: '#0E2A12',
...
const EditBuyer = () => {
  ...
  const fetchBuyer = async () => {
    ...
    try {
      const token = getToken();
      const response = await axios.get(`${BASE_URL}/buyers/${id}`, {
        headers: {
      ...
      if (response.data.success) {
        ...
        setFormData({
          ...
          email: buyer.email || '',
          mobile: buyer.mobile || '',
          alternateMobile: buyer.alternateMobile || '',
          ...
          businessType: buyer.businessType || 'individual',
          creditLimit: buyer.creditLimit || '',
          creditDays: buyer.creditDays || '',
          defaultPaymentMode: buyer.defaultPaymentMode || 'cash',
          notes: buyer.notes || ''
        });
      } else {
        setError(response.data.message || t('buyers.errors.fetchFailed'));
      }
    } catch (error) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await axios.put(`${BASE_URL}/buyers/${id}`, submitData, {
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
        }
      ...
      if (response.data.success) {
        ...
        const errorMessage = response.data.message || response.data.error || t('buyers.errors.updateFailed');
...
{loading ? <CircularProgress size={16} sx={{ color: 'white' }} /> : <><SaveIcon sx={{ fontSize: '1rem', mr: 0.5 }} /> {t('buyers.buttons.editBuyer')}</>}
      ...
      {success && (
        <Alert severity="success" sx={{ mb: 2, borderRadius: 2 }}>
          {t('buyers.messages.updateSuccess')}
        </Alert>
      )}
